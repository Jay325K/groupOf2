$(function(){
    let big_img = $('.space_main_img img');

    //메뉴 클릭시 이미지 바뀜
    $('.space_menu li>a').on('click focus',function(){
        $('.space_img_wrap').addClass('screen_out');
        $(this).next().removeClass('screen_out');
        let slice_src =$(this).next().children('a').eq(0).children().attr('src');
        let small_img =slice_src.slice(0,slice_src.length-4);
        big_img.attr('src',small_img+'_2.jpg');

        let wid =$(this).next().children('a').eq(0).children();
         if($(this).next().children('a').length>5){
            /* 아직은 이미지 개수가 5개가 넘지 않으므로 나중에 수정 */
            let count=0;
            $('space_wrap button').on('click',function(){
                if($(this).hasClass('prev')==true){
                    $('.space_menu').animate({'transform':'translateX('+-wid+'px)'});
                    count++;
                }else{
                    $('.space_menu').animate({'transform':'translateX('+wid+'px)'});
                    count--;
                }
            });
        } 
    });

    //작은 이미지 클릭시 큰 이미지 바뀜
    $('.space_img_wrap a img').on('click focus',function(){
        let img_src = $(this).attr('src').slice(0,$(this).attr('src').length-4);
        big_img.attr('src',(img_src+'_2.jpg'));
    })

    
})